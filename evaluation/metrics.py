...
def evaluator(gt_paths, pred_paths, metrics=['S', 'MAE', 'E', 'F', 'WF', 'MBA', 'BIoU', 'MSE', 'HCE', 'mAP', 'DICE', 'IOU', 'PREC', 'REC', 'SPEC', 'ACC'], verbose=False):
    # define measures
    if 'E' in metrics:
        EM = EMeasure()
    if 'S' in metrics:
    ...
    if 'BIoU' in metrics:
        BIoU = BIoUMeasure()
    common_metric_keys = {'DICE', 'IOU', 'PREC', 'REC', 'SPEC', 'ACC', 'mAP'}
    has_common_metrics = bool(common_metric_keys.intersection(set(metrics)))
    if has_common_metrics:
        CSM = CommonSegMeasure()
    ...
    for idx_sample in tqdm(range(len(gt_paths)), total=len(gt_paths)) if verbose else range(len(gt_paths)):
        ...
        if has_common_metrics:
            CSM.step(pred=pred_ary, gt=gt_ary)
    ...
    if has_common_metrics:
        common = CSM.get_results()['common']
    else:
        common = {
            'map': np.float64(-1),
            'dice': {'curve': np.array([np.float64(-1)]), 'adp': np.float64(-1), 'max': np.float64(-1), 'mean': np.float64(-1)},
            'iou': {'curve': np.array([np.float64(-1)]), 'adp': np.float64(-1), 'max': np.float64(-1), 'mean': np.float64(-1)},
            'precision': {'curve': np.array([np.float64(-1)]), 'adp': np.float64(-1), 'max': np.float64(-1), 'mean': np.float64(-1)},
            'recall': {'curve': np.array([np.float64(-1)]), 'adp': np.float64(-1), 'max': np.float64(-1), 'mean': np.float64(-1)},
            'specificity': {'curve': np.array([np.float64(-1)]), 'adp': np.float64(-1), 'max': np.float64(-1), 'mean': np.float64(-1)},
            'accuracy': {'curve': np.array([np.float64(-1)]), 'adp': np.float64(-1), 'max': np.float64(-1), 'mean': np.float64(-1)},
        }

    return em, sm, fm, mae, mse, wfm, hce, mba, biou, common


class CommonSegMeasure(object):
    def __init__(self):
        self.dice_curves = []
        self.iou_curves = []
        self.precision_curves = []
        self.recall_curves = []
        self.specificity_curves = []
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray):
        ...
        curves = self.cal_curve_metrics(pred, gt)
        self.dice_curves.append(curves['dice'])
        self.iou_curves.append(curves['iou'])
        self.precision_curves.append(curves['precision'])
        self.recall_curves.append(curves['recall'])
        self.specificity_curves.append(curves['specificity'])
        self.accuracy_curves.append(curves['accuracy'])

        adp = self.cal_adaptive_metrics(pred, gt)
        self.adp_dices.append(adp['dice'])
        self.adp_ious.append(adp['iou'])
        self.adp_precisions.append(adp['precision'])
        self.adp_recalls.append(adp['recall'])
        self.adp_specificities.append(adp['specificity'])
        self.adp_accuracies.append(adp['accuracy'])

        ap = self.cal_ap(curves['precision'], curves['recall'])
        self.aps.append(ap)

    def cal_curve_metrics(self, pred: np.ndarray, gt: np.ndarray) -> dict:
        pred_uint8 = (pred * 255).astype(np.uint8)
        bins = np.linspace(0, 256, 257)

        fg_hist, _ = np.histogram(pred_uint8[gt], bins=bins)
        bg_hist, _ = np.histogram(pred_uint8[~gt], bins=bins)
        tp = np.cumsum(np.flip(fg_hist), axis=0).astype(np.float64)
        fp = np.cumsum(np.flip(bg_hist), axis=0).astype(np.float64)

        t = float(max(np.count_nonzero(gt), 1))
        n = float(gt.size)
        fn = t - tp
        tn = (n - t) - fp
        ...
        recall = tp / t
        specificity = tn / np.maximum(tn + fp, _EPS)
        accuracy = (tp + tn) / n
        iou = tp / np.maximum(tp + fp + fn, _EPS)
        dice = (2 * tp) / np.maximum(2 * tp + fp + fn, _EPS)
    ...
    def cal_adaptive_metrics(self, pred: np.ndarray, gt: np.ndarray) -> dict:
        threshold = _get_adaptive_threshold(pred, max_value=1)
        bin_pred = pred >= threshold

        tp = float(np.count_nonzero(bin_pred & gt))
        fp = float(np.count_nonzero(bin_pred & ~gt))
        fn = float(np.count_nonzero((~bin_pred) & gt))
        tn = float(np.count_nonzero((~bin_pred) & (~gt)))

        precision = tp / max(tp + fp, _EPS)
        recall = tp / max(tp + fn, _EPS)
        specificity = tn / max(tn + fp, _EPS)
        accuracy = (tp + tn) / max(tp + tn + fp + fn, _EPS)
        iou = tp / max(tp + fp + fn, _EPS)
        dice = (2 * tp) / max(2 * tp + fp + fn, _EPS)
    ...
    def cal_ap(self, precision: np.ndarray, recall: np.ndarray) -> float:
    ...
    def _pack_metric(self, curves, adps) -> dict:
        curve = np.mean(np.array(curves, dtype=_TYPE), axis=0)
        adp = np.mean(np.array(adps, dtype=_TYPE))
        return {
            'curve': curve,
            'adp': adp,
            'max': curve.max(),
    ...
    def get_results(self) -> dict:
        common = {
            'map': np.mean(np.array(self.aps, dtype=_TYPE)),
            'dice': self._pack_metric(self.dice_curves, self.adp_dices),
            'iou': self._pack_metric(self.iou_curves, self.adp_ious),
            'precision': self._pack_metric(self.precision_curves, self.adp_precisions),
            'recall': self._pack_metric(self.recall_curves, self.adp_recalls),
            'specificity': self._pack_metric(self.specificity_curves, self.adp_specificities),
            'accuracy': self._pack_metric(self.accuracy_curves, self.adp_accuracies),
        }
