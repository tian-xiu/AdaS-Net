...
def init_models_optimizers(epochs, to_be_distributed):
    ...
    if str(config.model).lower() == 'adasnet':
        model = AdasNet(bb_pretrained=True and not os.path.isfile(str(args.resume)))
    else:
...
class Trainer:
    ...
    def train_epoch(self, epoch):
        ...
        if epoch > args.epochs + config.finetune_last_epochs:
            if 'bce' in self.pix_loss.lambdas_pix_last:
                self.pix_loss.lambdas_pix_last['bce'] *= 0
            if 'ssim' in self.pix_loss.lambdas_pix_last:
                self.pix_loss.lambdas_pix_last['ssim'] *= 1
            if 'iou' in self.pix_loss.lambdas_pix_last:
                self.pix_loss.lambdas_pix_last['iou'] *= 0.5
            if 'mae' in self.pix_loss.lambdas_pix_last:
