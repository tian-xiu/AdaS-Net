...
from models.backbones.MambaVision_T_1K import mamba_vision_t_1k
from models.backbones.res2net import res2net50_26w_4s
...
def build_backbone(bb_name, pretrained=True, params_settings=''):
    if bb_name == 'vgg16':
        ...
        if pretrained:
            if bb_name in config.weights and config.weights[bb_name] and os.path.isfile(config.weights[bb_name]):
                bb = load_weights(bb, bb_name)
            else:
                print('Skip loading pretrained weights for {}: no local weights path configured.'.format(bb_name))
    return bb
