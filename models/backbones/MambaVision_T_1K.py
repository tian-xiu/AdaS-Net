...
# def mambavision_t(num_classes=1000, pretrained=False):
#     model = MambaVision(
#         dim=80,
#         in_dim=32,
#         depths=[1, 3, 8, 4],
...
#     if pretrained:
#         # 加载预训练权重（可选）
#         model = load_weights(model, model_name="mambavision_t")

#     return model

def mambavision_t(num_classes=1000):
    model = MambaVision(
        dim=80,
        in_dim=32,
        depths=[1, 3, 8, 4],
...
class MambaVisionT1KBackbone(nn.Module):
    def __init__(self):
        super().__init__()
        self.model = mambavision_t(num_classes=1000)

    def forward(self, x):
        _, features = self.model.forward_features(x)
        return tuple(features)


def mamba_vision_t_1k():
    return MambaVisionT1KBackbone()
